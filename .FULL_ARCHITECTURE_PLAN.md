...
- [ ] Define Go `Provisioner` interface: `Create(ctx, UserID) (Instance, error)`, `Destroy(ctx, InstanceID)`, `Status(ctx, InstanceID)`, `Pause(ctx, InstanceID)`, `Wake(ctx, InstanceID)`
- [ ] `Instance` struct: ID, UserID, Provider ("docker"|"hetzner"), Host, Port, Status, VolumeID
- [ ] Config: `PROVIDER` env var selects active backend (default: `docker`)
- [ ] Provider registry: `NewProvisioner(cfg) Provisioner` factory
*Effort: M | Owner: You*

**Feature 1.1.2: Docker Provider**
- [ ] Implement `Provisioner` for local Docker (via Docker SDK for Go)
- [ ] `Create`: spin up container from `claude-instance` image, bind-mount named volume to `/claude-data`
- [ ] Dockerfile for `claude-instance`: Ubuntu base, Claude Code installed, zellij, mosh-server
- [ ] Container gets `dangerously-approve-all` in `~/.config/claude/settings.json`
- [ ] Start detached Zellij session on container boot (entrypoint script)
- [ ] Docker network `claude-net` for inter-container connectivity (bridge mode)
- [ ] `Pause`/`Wake`: `docker stop`/`docker start` (volume persists)
- [ ] `Destroy`: remove container, optionally remove volume
- [ ] Test: `Create` → container running, `docker exec` into Zellij session works
*Effort: L | Owner: You*

**Feature 1.1.3: Docker Compose Dev Stack**
- [ ] `docker-compose.yml`: Postgres + control plane API + one test claude-instance
- [ ] `.env.example` with `PROVIDER=docker` and Postgres DSN
- [ ] `make dev` target to bring up full local stack
...
- [ ] Set up Terraform workspace (GitHub repo: `claude-cloud-infra`) with `.tfvars` for secrets
- [ ] Configure providers: `hcloud` (v1.48+), `netbird` (if official), `random`, `time`
...
- [ ] Terraform: `hcloud_network` → `claude-private-net` (CIDR `10.100.0.0/16`)
...
- [ ] Implement `Provisioner` for Hetzner (via `terraform-exec` or direct hcloud Go SDK)
- [ ] `Create`: Terraform module `modules/user_instance` (server, volume, attachment, no public IP)
- [ ] Dynamic subnet assignment: `10.100.${user_id % 250 + 1}.10` (round-robin)
- [ ] Cloud-init template with Netbird setup + Claude Code install
- [ ] `Pause`: snapshot server, destroy server (volume remains)
- [ ] `Wake`: recreate from snapshot, re-attach volume
...
- [ ] New repo: `claude-cloud-backend` (Go)
- [ ] Docker + Postgres (local for dev, Railway or Hetzner CX11 for prod)
- [ ] Ent schema: User, Subscription, Instance (provider, host, port, volume_id, status)
- [ ] Auth: Clerk or Supabase Auth (email + magic link)
*Effort: L | Owner: You*

**Feature 1.3.2: Instance API**
- [ ] `POST /instances` → calls active `Provisioner.Create()`
- [ ] `DELETE /instances/:id` → calls `Provisioner.Destroy()`
- [ ] `POST /instances/:id/pause` → calls `Provisioner.Pause()`
- [ ] `POST /instances/:id/wake` → calls `Provisioner.Wake()`
- [ ] `GET /instances/:id` → calls `Provisioner.Status()`
- [ ] All endpoints are provider-agnostic (same API whether Docker or Hetzner)
...
- [ ] Netbird Go client wrapper (or direct HTTP)
- [ ] On user signup (when `PROVIDER=hetzner`):
...
- [ ] Cron job: cleanup expired keys
- [ ] Skip all Netbird steps when `PROVIDER=docker` (containers are directly reachable)
*Effort: M | Owner: You*

**Feature 2.1.2: User Connect Script**
- [ ] `/connect.sh` endpoint (returns bash script, provider-aware)
- [ ] Docker mode: `docker exec -it claude-{user_id} zellij attach claude`
- [ ] Hetzner mode: `curl netbird install | bash && netbird up --setup-key $KEY`, alias `claude` → `mosh root@10.100.x.10`
*Effort: S | Owner: You*

#### **Epic 2.2: Instance Bootstrap**
**Feature 2.2.1: Claude Code + Persistent Session (shared logic)**
- [ ] Entrypoint script (used by both Dockerfile and cloud-init):
  - Install/verify mosh, zellij
  - Install Claude Code (official script)
  - Set `dangerously-approve-all` in `~/.config/claude/settings.json`
  - Start detached Zellij session `claude` with layout: 1 pane Claude, 1 pane shell
- [ ] Volume mount: `/claude-data` (Docker named volume or Hetzner block volume)
- [ ] Docker-specific: expose mosh port on container
- [ ] Hetzner-specific: `ufw allow from 10.100.0.1` (only proxy)
*Effort: M | Owner: You*

**Feature 2.2.2: Inactivity Detection Agent**
- [ ] Tiny Go binary on instance: every 30s POST heartbeat to control plane (last_activity)
- [ ] Docker mode: agent runs inside container, reports to `http://host.docker.internal:PORT`
- [ ] Hetzner mode: agent reports to control plane via private net
- [ ] Or: poll container/peer status from control plane side
*Effort: M | Owner: You*
...
- [ ] Stripe products: Base ($19/mo, 20h included), Usage ($0.08/h)
- [ ] Webhook: on payment → create user record + trigger instance provisioning
- [ ] Portal: Next.js app showing "My Claude" status (Running / Paused)
*Effort: L | Owner: You*

**Feature 3.1.2: "Wake My Claude" Button**
- [ ] Calls `Provisioner.Wake()` / `Provisioner.Status()` — provider-agnostic
- [ ] Docker: `docker start` container (volume persists)
- [ ] Hetzner: recreate from snapshot, re-attach volume, re-apply Netbird route/policy
- [ ] If running: just show connection details
*Effort: M | Owner: You*

#### **Epic 3.2: CLI Experience Polish**
**Feature 3.2.1: One-Command Connect**
- [ ] `claude-cloud connect` (downloads Netbird config, opens mosh)
...
#### **Epic 4.1: Web Terminal & Projects**
**Feature 4.1.1: OptimusHQ Fork or Custom**
- [ ] Deploy OptimusHQ (open-source Claude dashboard) behind Netbird
- [ ] Custom theme + your branding
- [ ] Projects: Git repos auto-cloned into volume
*Effort: L | Owner: You (or hire freelancer)*

**Feature 4.1.2: Web Terminal (ttyd)**
- [ ] Run ttyd on VPS → expose via Netbird
- [ ] "Open Terminal" button in dashboard
*Effort: M | Owner: You*

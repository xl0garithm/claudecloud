...
- [x] Structured logging: slog (JSON in prod, text in dev) with trace correlation
- [x] OpenTelemetry tracing: OTLP HTTP exporter, auto-instrumented HTTP, manual spans on business ops
- [x] Prometheus metrics: `/metrics` endpoint, instance count meters, request duration histograms
- [x] Process supervision: supervisor.sh with exponential backoff restarts
- [x] Instance health monitoring: Docker HEALTHCHECK, consecutive failure tracking
*Effort: M | Owner: You*

**Feature 5.1.2: Security & Hardening**
- [x] Security headers middleware: X-Content-Type-Options, X-Frame-Options, HSTS, CSP-adjacent
- [x] Rate limiting: per-IP token bucket (5/min auth, 60/min API)
- [x] Request body size limit: 1MB via http.MaxBytesReader
- [x] Config validation: fail-fast in production if JWT_SECRET/DATABASE_URL missing
- [x] Enhanced health check: DB ping, version reporting, 503 on failure
*Effort: M | Owner: You*

#### **Epic 5.2: Launch**
**Feature 5.2.1: Landing Page & Marketing**
- [x] Expanded landing page: hero, how-it-works, feature grid, FAQ accordion, footer
- [x] PricingCard "Most Popular" badge support

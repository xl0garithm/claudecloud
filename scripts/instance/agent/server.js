...
// --- Zellij helper: run an action targeting the "main" session ---
const ZELLIJ_SESSION = "main";

function zellijAction(args, callback) {
  execFile(
    "zellij",
    ["-s", ZELLIJ_SESSION, "action", ...args],
    { timeout: 5000 },
    callback
  );
}
...
app.post("/tabs", (req, res) => {
  ...
  zellijAction(["query-tab-names"], (err, stdout) => {
    ...
    if (tabExists) {
      ...
      zellijAction(["go-to-tab-name", name], () => {
        // Write the claude command into the pane if resuming
        if (resume) {
          zellijAction(["write-chars", claudeCmd + "\n"], () => {});
        }
    ...
    fs.mkdir(tabCwd, { recursive: true }, () => {
      // Create a new tab
      zellijAction(
        ["new-tab", "--name", name, "--cwd", tabCwd],
        (err) => {
          if (err) {
            return res.status(500).json({ error: "failed to create tab: " + err.message });
          }

          // Small delay to let the shell in the new pane initialize,
          // then type the claude command
          setTimeout(() => {
            zellijAction(["write-chars", claudeCmd + "\n"], () => {});
          }, 500);
...
app.delete("/tabs/:name", (req, res) => {
  ...
  zellijAction(["go-to-tab-name", name], (err) => {
    if (err) {
      return res.status(404).json({ error: "tab not found" });
    }

    // Close the tab (kills all panes/processes in it)
    zellijAction(["close-tab"], (err) => {
      if (err) {
    ...
    projectTabs.forEach((tabName) => {
      execFile("pgrep", ["-f", `claude.*${tabName}`], { timeout: 3000 }, (err) => {
        ...
        if (idleDuration >= IDLE_TIMEOUT_MS) {
          ...
          zellijAction(["go-to-tab-name", tabName], () => {

...
func NewNetbirdService(client *netbird.Client, logger *log.Logger) *NetbirdService {
...
func (s *NetbirdService) PrepareNetbirdAccess(ctx context.Context, userID int) (*NetbirdPrep, error) {
	...
	s.logger.Printf("netbird: created group %s for user %d", group.ID, userID)
	...
	s.logger.Printf("netbird: created setup key %s for user %d", key.ID, userID)
...
func (s *NetbirdService) FinalizeNetbirdAccess(ctx context.Context, userID int, prep *NetbirdPrep) (*UserNetbirdConfig, error) {
	...
	s.logger.Printf("netbird: created route %s for user %d", route.ID, userID)
	...
	s.logger.Printf("netbird: created policy %s for user %d", policy.ID, userID)
...
func (s *NetbirdService) TeardownUser(ctx context.Context, cfg *UserNetbirdConfig) error {
	...
	if cfg.PolicyID != "" {
		if err := s.client.DeletePolicy(ctx, cfg.PolicyID); err != nil {
			s.logger.Printf("netbird: failed to delete policy %s: %v", cfg.PolicyID, err)
			if firstErr == nil {
				firstErr = err
			}
		}
	...
	if cfg.RouteID != "" {
		if err := s.client.DeleteRoute(ctx, cfg.RouteID); err != nil {
			s.logger.Printf("netbird: failed to delete route %s: %v", cfg.RouteID, err)
			if firstErr == nil {
				firstErr = err
			}
		}
	...
	if cfg.GroupID != "" {
		if err := s.client.DeleteGroup(ctx, cfg.GroupID); err != nil {
			s.logger.Printf("netbird: failed to delete group %s: %v", cfg.GroupID, err)
			if firstErr == nil {
				firstErr = err
			}
		}
	...
	for _, key := range keys {
		if !key.Valid && !key.Revoked {
			if err := s.client.RevokeSetupKey(ctx, key.ID); err != nil {
				s.logger.Printf("netbird: failed to revoke expired key %s: %v", key.ID, err)
				continue
			}
			s.logger.Printf("netbird: revoked expired key %s", key.ID)
		}
	}
